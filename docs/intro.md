...
If you want to translate the labels, see the [Localization](./localization) guide. Otherwise, just bump the version:
...
1. **You're not on a doc page.** Auto-injection only targets pages rendered by `@docusaurus/plugin-content-docs`. The homepage (`src/pages/index.tsx`) and other custom pages don't get the button by default — use the [Swizzled placement](./recipes/swizzled-placement) recipe to put it on custom pages.
...
3. **You set `injectButton: false` but didn't render the component.** Auto-injection is required unless you're manually rendering. Set it back to the default (`true`) or follow [Swizzled placement](./recipes/swizzled-placement).
...
`View as Markdown` creates a `Blob` URL and opens it. Some sandboxed environments (corporate browsers, certain iframes) block `blob:` URLs. Workaround: set [`generateMarkdownRoutes: true`](./recipes/markdown-routes) so the markdown is served from a real URL instead of a blob.
...
2. Your `i18n/<locale>/code.json` has the plugin's IDs. `docusaurus write-translations` does **not** extract them from `node_modules` — you have to add them manually. See the [Localization](./localization) page for the full ID table.

## Where next

- [Configuration](./configuration) — every option in one place
- [Localization](./localization) — translate the button labels
- [Recipes](./recipes/private-docs) — copy-paste examples for common setups
- [How It Works](./how-it-works) — the markdown extractor + AI deep-link mechanics
